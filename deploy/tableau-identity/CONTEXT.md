...
2. **1.4 — единая админка с ролями — СДЕЛАНО (Уровень 2, без Keycloak).**
   Новый сервис `admin-console` (`deploy/admin-console/`): один вход
   (`:8030`), логин по роли → доступ ТОЛЬКО к разрешённым разделам. Бэкенд-
   админки сняты с хост-портов и проксируются с ролевым гейтингом:
   `analyst` → контекст+презентации, `access-admin` → PAT-привязки, `admin` →
   всё. Разделение жёсткое: аналитик получает **403** на раздел PAT (не просто
   скрытую ссылку). Реализовано без правки бэкендов (реверс-прокси +
   переписывание корневых ссылок/редиректов под префикс раздела). Уровень 3
   (единый вход через OAuth/токен гейта) — в фазе Keycloak.
3. **Версионирование бизнес-контекста** (ТС 4.5 / 5.6.3) — **СДЕЛАНО.**
   dashboard-context-admin на каждом сохранении снимает копию каталога в
   `catalog/history/` (файловая история, air-gap-friendly), в UI — раздел
   «History» (весь каталог) + на странице записи **таймлайн изменений этой
   записи** (что менялось vs сейчас) с точечным restore. Restore обратим.
   Хранятся последние `CONTEXT_HISTORY_KEEP` (дефолт 50).
